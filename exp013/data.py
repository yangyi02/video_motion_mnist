...
def generate_images(args, images, m_dict, reverse_m_dict):
    if numpy.random.rand() < 0.66:
        im1, im2, im3, _ = generate_moving_images(args, images, m_dict, reverse_m_dict)
    else:
        im1, im2, im3 = generate_rotating_images(args, images, m_dict, reverse_m_dict)
    return im1, im2, im3


def generate_rotating_images(args, images, m_dict, reverse_m_dict):
    noise = 0.5
    min_angle = 1
    max_angle = 15
    ...
    angle = numpy.random.randint(min_angle, max_angle, size=batch_size)
    reverse_angle = numpy.random.randint(0, 2, size=batch_size)
    angle[reverse_angle == 1] = -angle[reverse_angle == 1]
    bg = numpy.random.rand(batch_size, im_channel, im_size, im_size) * noise
    im1 = rotate_image(im2, -angle)
    im3 = rotate_image(im2, angle)
    im1[im1 == 0] = bg[im1 == 0]
    im2[im2 == 0] = bg[im2 == 0]
    im3[im3 == 0] = bg[im3 == 0]
    if False:
...
def rotate_image(im, angle):
    ...
    for i in range(batch_size):
        ...
        im_new[i, 0, :, :] = cv2.warpAffine(im[i, :, :, :].squeeze(), M, (im_size, im_size))
    return im_new
