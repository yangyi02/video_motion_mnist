...
def generate_images(args, images, m_dict, reverse_m_dict):
    ...
    im2 = move_image_fg(im3, m_b_x[:, 0], m_b_y[:, 0], m_range)
    im1 = move_image_fg(im2, m_b_x[:, 0], m_b_y[:, 0], m_range)
    im4 = move_image_fg(im3, m_f_x[:, 0], m_f_y[:, 0], m_range)
    im5 = move_image_fg(im4, m_f_x[:, 0], m_f_y[:, 0], m_range)
    bg2 = move_image_bg(bg3, m_b_x[:, 1], m_b_y[:, 1], m_range)
    ...
    im5[im5 == 0] = bg5[im5 == 0]
    if False:
        display(im1, im2, im3, im4, im5, gt_motion_f, gt_motion_b)
