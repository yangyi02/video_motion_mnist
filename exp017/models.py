...
class UNet3(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        x = self.bn0(self.conv0(x))
        x1 = F.relu(self.bn1(self.conv1(x)))
        x1 = F.relu(x1 + self.bn1_2(self.conv1_2(x1)))
        x2 = self.maxpool(x1)
        x2 = F.relu(self.bn2(self.conv2(x2)))
        x2 = F.relu(x2 + self.bn2_2(self.conv2_2(x2)))
        x3 = self.maxpool(x2)
        x3 = F.relu(self.bn3(self.conv3(x3)))
        x3 = F.relu(x3 + self.bn3_2(self.conv3_2(x3)))
        x4 = self.maxpool(x3)
        x4 = F.relu(self.bn4(self.conv4(x4)))
        x4 = F.relu(x4 + self.bn4_2(self.conv4_2(x4)))
        x5 = self.maxpool(x4)
        x5 = F.relu(self.bn5(self.conv5(x5)))
        x5 = F.relu(x5 + self.bn5_2(self.conv5_2(x5)))
        x6 = self.maxpool(x5)
        x6 = F.relu(self.bn6(self.conv6(x6)))
        x6 = F.relu(x6 + self.bn6_2(self.conv6_2(x6)))
        x6 = self.upsample(x6)
        x7 = torch.cat((x6, x5), 1)
        x7 = F.relu(self.bn7(self.conv7(x7)))
        x7 = F.relu(x7 + self.bn7_2(self.conv7_2(x7)))
        x7 = self.upsample(x7)
        x8 = torch.cat((x7, x4), 1)
        x8 = F.relu(self.bn8(self.conv8(x8)))
        x8 = F.relu(x8 + self.bn8_2(self.conv8_2(x8)))
        x8 = self.upsample(x8)
        x9 = torch.cat((x8, x3), 1)
        x9 = F.relu(self.bn9(self.conv9(x9)))
        x9 = F.relu(x9 + self.bn9_2(self.conv9_2(x9)))
        x9 = self.upsample(x9)
        x10 = torch.cat((x9, x2), 1)
        x10 = F.relu(self.bn10(self.conv10(x10)))
        x10 = F.relu(x10 + self.bn10_2(self.conv10_2(x10)))
        x10 = self.upsample(x10)
        x = torch.cat((x10, x1), 1)
        motion = self.conv(x)
        return motion
...
class UNet2(nn.Module):
    def __init__(self, im_size, im_channel, n_class):
        ...
        self.conv7 = nn.Conv2d(num_hidden*32, num_hidden*8, 3, 1, 1)
        self.bn7 = nn.BatchNorm2d(num_hidden*8)
        self.conv8 = nn.Conv2d(num_hidden*16, num_hidden*4, 3, 1, 1)
        self.bn8 = nn.BatchNorm2d(num_hidden*4)
        self.conv9 = nn.Conv2d(num_hidden*8, num_hidden*2, 3, 1, 1)
        self.bn9 = nn.BatchNorm2d(num_hidden*2)
        self.conv10 = nn.Conv2d(num_hidden*4, num_hidden, 3, 1, 1)
        self.bn10 = nn.BatchNorm2d(num_hidden)
        self.maxpool = nn.MaxPool2d(2, stride=2, return_indices=False, ceil_mode=False)
        self.upsample = nn.UpsamplingBilinear2d(scale_factor=2)
        self.conv = nn.Conv2d(num_hidden*2, n_class, 3, 1, 1)
