...
def test_supervised(args, model, images, m_dict, reverse_m_dict, m_kernel):
    ...
    for epoch in range(args.test_epoch):
        im1, im2, im3 = generate_images(args, images, m_dict, reverse_m_dict)
        im1 = Variable(torch.from_numpy(im1).float())
        im2 = Variable(torch.from_numpy(im2).float())
        ...
        if args.display:
            ...
            # visualize(im1, im2, im3, pred, pred_motion, m_range, m_dict, reverse_m_dict)
            motion = F.softmax(motion)
            flow = label2flow2(motion, reverse_m_dict)
            visualize(im1, im2, im3, pred, pred_motion, flow, m_range, m_dict, reverse_m_dict)
...
def label2flow2(motion, reverse_m_dict):
