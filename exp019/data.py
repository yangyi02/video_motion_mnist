...
def generate_images(args, images, m_dict, reverse_m_dict):
    ...
    im2 = images[idx[0:batch_size], :, :, :]
    bg2 = numpy.random.rand(batch_size, im_channel, im_size, im_size) * noise
    m_label = numpy.random.randint(0, len(m_dict), size=(batch_size, 2))
    m_f_x = numpy.zeros((batch_size, 2)).astype(int)
    m_f_y = numpy.zeros((batch_size, 2)).astype(int)
    m_b_x = numpy.zeros((batch_size, 2)).astype(int)
    m_b_y = numpy.zeros((batch_size, 2)).astype(int)
    ...
    for i in range(batch_size):
        gt_motion_f[i, :, :, :] = m_label[i, 0]
        gt_motion_b[i, :, :, :] = m_dict[(m_b_x[i, 0], m_b_y[i, 0])]
        bg_motion_f[i, :, :, :] = m_label[i, 1]
        bg_motion_b[i, :, :, :] = m_dict[(m_b_x[i, 1], m_b_y[i, 1])]
    im1 = move_image_fg(im2, m_b_x[:, 0], m_b_y[:, 0], m_range)
    im3 = move_image_fg(im2, m_f_x[:, 0], m_f_y[:, 0], m_range)
    bg1 = move_image_bg(bg2, m_b_x[:, 1], m_b_y[:, 1], m_range)
    bg3 = move_image_bg(bg2, m_f_x[:, 1], m_f_y[:, 1], m_range)
    for i in range(batch_size):
        gt_motion_f[im1 == 0] = bg_motion_f[im1 == 0]
        gt_motion_b[im3 == 0] = bg_motion_b[im3 == 0]
    im1[im1 == 0] = bg1[im1 == 0]
    im2[im2 == 0] = bg2[im2 == 0]
    im3[im3 == 0] = bg3[im3 == 0]
    if False:
        display(im1, im2, im3, gt_motion_f, gt_motion_b)
    return im1, im2, im3, gt_motion_f.astype(int), gt_motion_b.astype(int)
...
def display(im1, im2, im3, gt_motion_f, gt_motion_b):
    plt.figure(1)
    plt.subplot(1, 3, 1)
