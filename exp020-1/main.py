...
def test_unsupervised(args, model, m_dict, reverse_m_dict, m_kernel):
    ...
    for epoch in range(args.test_epoch):
        ...
        for i in range(1, 2):
            ...
            pred = construct_image(im_input_last, motion, m_range, m_kernel, padding=0)
            ...
            if args.display:
                m_range = args.motion_range
                pred = construct_image(im_input_last, motion, m_range, m_kernel, padding=m_range)
                pred_motion = motion.max(1)[1]
                visualize(im_input_last, im_output, pred, pred_motion, F.softmax(motion), m_range, m_dict, reverse_m_dict)
...
def main():
    ...
    model = UNet(args.image_height, args.image_width, im_channel, len(m_dict) + 1)
    if torch.cuda.is_available():
        model = model.cuda()
