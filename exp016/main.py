...
def train_supervised(args, model, images, m_dict, reverse_m_dict, m_kernel):
    ...
    for epoch in range(args.train_epoch):
        ...
        pred, pred_f, motion_f, pred_b, motion_b, attn_f, attn_b = model(im1, im3, ones)
...
def test_unsupervised(args, model, images, m_dict, reverse_m_dict, m_kernel):
    ...
    for epoch in range(args.test_epoch):
        ...
        if args.display:
            m_range = args.motion_range
            visualize(im1, im2, im3, pred, pred_motion_f, gt_motion_f, attn_f, pred_motion_b, gt_motion_b, attn_b, m_range, m_dict, reverse_m_dict)
