...
def train_supervised(args, model, motion_range, m_dict, reverse_m_dict):
    optimizer = optim.Adam(model.parameters(), lr=args.learning_rate)
    for epoch in range(args.train_epoch):
        optimizer.zero_grad()
        im1, im2, gt_motion = data.generate_images(args, m_dict, reverse_m_dict, model.im_size, motion_range)
...
def test_supervised(args, model, motion_range, m_dict, reverse_m_dict):
    test_accuracy = []
...
def train_unsupervised(args, model, motion_range, m_dict, reverse_m_dict):
    optimizer = optim.Adam(model.parameters(), lr=args.learning_rate)
    for epoch in range(args.train_epoch):
        optimizer.zero_grad()
        im1, im2, gt_motion = data.generate_images(args, m_dict, reverse_m_dict, model.im_size, motion_range)
...
def main():
    ...
    logging.info(args)
    m_dict, reverse_m_dict = data.motion_dict(args.motion_range)
    model = models.Net(args.image_size, len(m_dict))
    if torch.cuda.is_available():
        model = model.cuda()
    if args.train:
        if args.method == 'supervised':
            model = train_supervised(args, model, args.motion_range, m_dict, reverse_m_dict)
        else:
            model = train_unsupervised(args, model, args.motion_range, m_dict, reverse_m_dict)
    ...
    if args.test:
        ...
        test_accuracy = test_supervised(args, model, args.motion_range, m_dict, reverse_m_dict)
