...
class UNetBidirection(nn.Module):
    ...
    def forward(self, im1, im3, ones):
        ...
        pred_f = construct_image(im1, motion_f, disappear_f, self.m_range, self.m_kernel, padding=self.m_range)
        pred_b = construct_image(im3, motion_b, disappear_b, self.m_range, self.m_kernel, padding=self.m_range)
