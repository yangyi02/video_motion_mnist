...
def train_supervised(args, model, images, m_dict, reverse_m_dict, m_kernel):
    ...
    for epoch in range(args.train_epoch):
        ...
        pred, pred_f, motion_f, attn_f, pred_b, motion_b, attn_b = model(im1, im3, ones)
...
def test_unsupervised(args, model, images, m_dict, reverse_m_dict, m_kernel):
    ...
    for epoch in range(args.test_epoch):
        ...
        if args.display:
            ...
            disappear_f = m_mask_f[:, model.n_class - 1, :, :].unsqueeze(1)
            m_mask_b = F.softmax(motion_b)
            flow_b = motion2flow(m_mask_b, reverse_m_dict)
            disappear_b = m_mask_b[:, model.n_class - 1, :, :].unsqueeze(1)
            visualize(im1, im2, im3, pred, flow_f, gt_motion_f, disappear_f, attn_f, flow_b, gt_motion_b, disappear_b, attn_b, m_range, reverse_m_dict)
