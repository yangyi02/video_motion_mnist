...
def generate_images(m_dict, reverse_m_dict, im_size, im_channel, motion_range, images, batch_size=32):
    ...
    im_big[:, :, motion_range:-motion_range, motion_range:-motion_range] = im3
    im4 = numpy.zeros((batch_size, im_channel, im_size, im_size))
    for i in range(batch_size):
        (motion_x, motion_y) = reverse_m_dict[motion_label[i]]
        im4[i, :, :, :] = im_big[i, :, motion_range + motion_y:motion_range+motion_y+im_size,
                          motion_range+motion_x:motion_range+motion_x+im_size]
    bg_big = numpy.random.rand(batch_size, im_channel, im_size + motion_range * 2,
                               im_size + motion_range * 2) * noise_magnitude
    bg_big[:, :, motion_range:-motion_range, motion_range:-motion_range] = bg3
    bg4 = numpy.zeros((batch_size, im_channel, im_size, im_size))
    for i in range(batch_size):
        (bg_motion_x, bg_motion_y) = reverse_m_dict[bg_motion_label[i]]
        bg4[i, :, :, :] = bg_big[i, :,
    ...
    gt_motion1[im2 == 0] = bg_gt_motion1[im2 == 0]
    gt_motion2[im3 == 0] = bg_gt_motion2[im3 == 0]
    im1[im1 == 0] = bg1[im1 == 0]
    im2[im2 == 0] = bg2[im2 == 0]
    im3[im3 == 0] = bg3[im3 == 0]
    im4[im4 == 0] = bg4[im4 == 0]
    if show:
        ...
        plt.subplot(2,3,5)
        plt.imshow(gt_motion1[0, :, :, :].squeeze())
        plt.subplot(2,3,6)
        plt.imshow(gt_motion2[0, :, :, :].squeeze())
        plt.show()

    return im1, im2, im3, im4, gt_motion1.astype(int), gt_motion2.astype(int)
